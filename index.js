...
function filterStudentsByGrade(students, grade) {
  ...
  let filterStud = students.filter((student) => student.grade === grade);
...
function warnAboutMaxArrayLength(arr, maxLength) {
  ...
  let result = arr.length > maxLength;
  ...
  console.assert(
    result,
    "Увага! Довжина масиву не перевищує максимально допустиму!"
  );
}
...
function compareArrays(arr1, arr2) {
  ...
  let arr2J = JSON.stringify(arr2);

  // Використовуємо `console.assert()`, щоб вивести помилку, якщо масиви не однакові.
  console.assert(arr1J === arr2J, "Масиви не однакові");
}
...
function countVowelsAndConsonants(word) {
  ...
  for (let el of word) {
    let elSmall = el.toLowerCase();

    if (str.includes(elSmall)) {
      gol++;
    } else if (elSmall >= "a" && elSmall <= "z") {
      prig++;
    }
  }
  ...
  // Скидаємо лічильники
  console.log(`Кількість голосних: ${gol}`);
  console.log(`Кількість приголосних: ${prig}`);
}
